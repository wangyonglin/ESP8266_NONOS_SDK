...
void ICACHE_FLASH_ATTR OLED_Header_Clear(void){
		u8 page,row;
		for(page=0; page<2; page++)
		{
			OLED_WR_Byte(0xb0+page,OLED_CMD);	// 从0～7页依次写入
			OLED_WR_Byte(0x00,OLED_CMD);      	// 列低地址
			OLED_WR_Byte(0x10,OLED_CMD);      	// 列高地址
			for(row=0; row<128; row++)OLED_WR_Byte(0x00,OLED_DATA);
		}

}
void ICACHE_FLASH_ATTR OLED_Body_Clear(void)
{
		u8 N_Page, N_row;
		for(N_Page=2; N_Page<8; N_Page++)
		{
